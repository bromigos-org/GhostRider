...
def sms_config() -> SMSConfig:
    ...
    return SMSConfig(
        enabled=True, textbee_api_key="test_api_key", textbee_device_id="test_device_id", polling_interval=1
    )


@pytest.fixture
def app_config(sms_config: SMSConfig) -> GhostRiderConfig:
    """Create GhostRider configuration for testing."""
    return GhostRiderConfig(sms=sms_config, debug=True)


@pytest.fixture
def ghost_rider_app(app_config: GhostRiderConfig) -> GhostRiderApp:
...
class TestGhostRiderIntegration:
    ...
    @pytest.mark.asyncio
    async def test_app_start_sms_disabled(self, app_config: GhostRiderConfig) -> None:
