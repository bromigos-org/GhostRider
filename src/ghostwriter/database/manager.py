...
class DatabaseManager:
    ...
    async def initialize(self) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_messages_channel_timestamp ON discord_messages (channel_id, timestamp)"
            )
    ...
    def _encrypt_token(self, token: str) -> str:
        ...
        encrypted: bytes = self.cipher.encrypt(token.encode())  # type: ignore
        return encrypted.decode()

    def _decrypt_token(self, encrypted_token: str) -> str:
        """Decrypt a stored token."""
        decrypted: bytes = self.cipher.decrypt(encrypted_token.encode())  # type: ignore
    ...
    async def store_discord_token(self, token: DiscordToken) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            """,
                (
                    token.user_id,
                    self._encrypt_token(token.access_token),
                    self._encrypt_token(token.refresh_token),
        ...
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
                ...
                (message_id, channel_id, author_id, author_name, author_avatar, content, timestamp,
                 edited_timestamp, message_type, attachments, embeds, mentions, reply_to, raw_data,
