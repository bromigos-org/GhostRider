...
from .platforms.sms import TextBeeSMSPlatform, TextBeeConfig
...
class GhostRiderApp:
    ...
    async def start(self) -> None:
        ...
        for platform_name, platform in self.platforms.items():
            task = asyncio.create_task(
                self._monitor_platform(platform_name, platform)
            )
    ...
    async def _monitor_platform(self, platform_name: str, platform: Any) -> None:
        ...
        while self.running:
            try:
                ...
                if (hasattr(platform, 'config') and 
                        hasattr(platform.config, 'polling_interval')):
    ...
    async def _process_message_batch(self, batch: MessageBatch) -> None:
        ...
        try:
            ...
            for result in results:
                if result.success:
                    print(f"✅ Processed message {result.message_id[:8]}... "
                    ...
                    print(f"❌ Failed to process message "
                          f"{result.message_id[:8]}...: {result.error}")
    ...
    async def _handle_processed_messages(self, batch: MessageBatch, 
                                       results: Any) -> None:
        ...
        for message in batch.messages:
            ...
            if message.priority.value in ['high', 'urgent']:
                ...
                print(f"   Priority: {message.priority.value} "
                      f"(score: {message.urgency_score:.2f})")
